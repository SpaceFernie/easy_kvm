EESchema Schematic File Version 4
LIBS:kvm-cache
EELAYER 26 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 3 3
Title "USB Switching"
Date "2017-12-31"
Rev "1.0"
Comp "D. FERNIE"
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Connector:USB_A J1
U 1 1 5A441D63
P 1300 1150
F 0 "J1" H 1357 1620 50  0000 C CNN
F 1 "USB_A" H 1357 1527 50  0000 C CNN
F 2 "Custom:USB_A_SMD" H 1450 1100 50  0001 C CNN
F 3 "" H 1450 1100 50  0001 C CNN
	1    1300 1150
	1    0    0    -1  
$EndComp
$Comp
L Connector:USB_A J2
U 1 1 5A441DE1
P 1300 2350
F 0 "J2" H 1357 2820 50  0000 C CNN
F 1 "USB_A" H 1357 2727 50  0000 C CNN
F 2 "Custom:USB_A_SMD" H 1450 2300 50  0001 C CNN
F 3 "" H 1450 2300 50  0001 C CNN
	1    1300 2350
	1    0    0    -1  
$EndComp
$Comp
L Connector:USB_A J3
U 1 1 5A441E08
P 1300 3550
F 0 "J3" H 1357 4020 50  0000 C CNN
F 1 "USB_A" H 1357 3927 50  0000 C CNN
F 2 "Custom:USB_A_SMD" H 1450 3500 50  0001 C CNN
F 3 "" H 1450 3500 50  0001 C CNN
	1    1300 3550
	1    0    0    -1  
$EndComp
Wire Wire Line
	1650 2100 1650 2150
Wire Wire Line
	1650 2150 1600 2150
Wire Wire Line
	1650 3300 1650 3350
Wire Wire Line
	1650 3350 1600 3350
$Comp
L power:GND #PWR04
U 1 1 5A4420FF
P 1250 1600
F 0 "#PWR04" H 1250 1350 50  0001 C CNN
F 1 "GND" H 1254 1421 50  0000 C CNN
F 2 "" H 1250 1600 50  0001 C CNN
F 3 "" H 1250 1600 50  0001 C CNN
	1    1250 1600
	1    0    0    -1  
$EndComp
Wire Wire Line
	1250 1600 1250 1550
Connection ~ 1250 1550
Wire Wire Line
	1250 1550 1300 1550
Wire Wire Line
	1200 1550 1250 1550
$Comp
L power:GND #PWR05
U 1 1 5A442225
P 1250 2800
F 0 "#PWR05" H 1250 2550 50  0001 C CNN
F 1 "GND" H 1254 2621 50  0000 C CNN
F 2 "" H 1250 2800 50  0001 C CNN
F 3 "" H 1250 2800 50  0001 C CNN
	1    1250 2800
	1    0    0    -1  
$EndComp
Wire Wire Line
	1250 2800 1250 2750
$Comp
L power:GND #PWR06
U 1 1 5A442322
P 1250 4000
F 0 "#PWR06" H 1250 3750 50  0001 C CNN
F 1 "GND" H 1254 3821 50  0000 C CNN
F 2 "" H 1250 4000 50  0001 C CNN
F 3 "" H 1250 4000 50  0001 C CNN
	1    1250 4000
	1    0    0    -1  
$EndComp
Wire Wire Line
	1250 4000 1250 3950
Wire Wire Line
	1200 3950 1250 3950
Connection ~ 1250 3950
Wire Wire Line
	1250 3950 1300 3950
Wire Wire Line
	1200 2750 1250 2750
Connection ~ 1250 2750
Wire Wire Line
	1250 2750 1300 2750
$Comp
L power:GND #PWR012
U 1 1 5A443571
P 5250 3950
F 0 "#PWR012" H 5250 3700 50  0001 C CNN
F 1 "GND" H 5254 3771 50  0000 C CNN
F 2 "" H 5250 3950 50  0001 C CNN
F 3 "" H 5250 3950 50  0001 C CNN
	1    5250 3950
	1    0    0    -1  
$EndComp
Text Label 1600 1150 0    50   ~ 0
USB1+
Text Label 1600 1250 0    50   ~ 0
USB1-
Text Label 1600 2350 0    50   ~ 0
USB2+
Text Label 1600 2450 0    50   ~ 0
USB2-
Text Label 1600 3550 0    50   ~ 0
USB3+
Text Label 1600 3650 0    50   ~ 0
USB3-
Text Label 4700 1150 2    50   ~ 0
USB1A+
Text Label 4700 1250 2    50   ~ 0
USB1A-
Text Label 5750 1150 0    50   ~ 0
USB1B+
Text Label 5750 1250 0    50   ~ 0
USB1B-
Text Label 4700 2300 2    50   ~ 0
USB2A+
Text Label 4700 2400 2    50   ~ 0
USB2A-
Text Label 4700 3500 2    50   ~ 0
USB3A+
Text Label 4700 3600 2    50   ~ 0
USB3A-
Text Label 5750 2300 0    50   ~ 0
USB2B+
Text Label 5750 2400 0    50   ~ 0
USB2B-
Text Label 5750 3500 0    50   ~ 0
USB3B+
Text Label 5750 3600 0    50   ~ 0
USB3B-
Text GLabel 7925 1100 2    50   Input ~ 0
CTRL
$Comp
L power:GND #PWR019
U 1 1 5A46EBAB
P 7950 1450
F 0 "#PWR019" H 7950 1200 50  0001 C CNN
F 1 "GND" V 7955 1318 50  0000 R CNN
F 2 "" H 7950 1450 50  0001 C CNN
F 3 "" H 7950 1450 50  0001 C CNN
	1    7950 1450
	1    0    0    -1  
$EndComp
Text Label 7075 1100 2    50   ~ 0
USB1A+
Text Label 7075 1300 2    50   ~ 0
USB1B+
Text Label 7075 900  2    50   ~ 0
USB1+
Text Label 7075 1000 2    50   ~ 0
USB1-
Text Label 7075 1400 2    50   ~ 0
USB1B-
Text Label 7075 1200 2    50   ~ 0
USB1A-
$Comp
L power:VCC #PWR022
U 1 1 5A470C84
P 7975 850
F 0 "#PWR022" H 7975 700 50  0001 C CNN
F 1 "VCC" H 7992 1030 50  0000 C CNN
F 2 "" H 7975 850 50  0001 C CNN
F 3 "" H 7975 850 50  0001 C CNN
	1    7975 850 
	1    0    0    -1  
$EndComp
$Comp
L device:C C8
U 1 1 5A477518
P 8300 1050
F 0 "C8" V 8045 1050 50  0000 C CNN
F 1 "0.1uF" V 8138 1050 50  0000 C CNN
F 2 "Capacitors_SMD:C_0805_HandSoldering" H 8338 900 50  0001 C CNN
F 3 "" H 8300 1050 50  0001 C CNN
	1    8300 1050
	-1   0    0    1   
$EndComp
$Comp
L power:+5VD #PWR032
U 1 1 5A483809
P 1650 2100
F 0 "#PWR032" H 1650 1950 50  0001 C CNN
F 1 "+5VD" H 1664 2280 50  0000 C CNN
F 2 "" H 1650 2100 50  0001 C CNN
F 3 "" H 1650 2100 50  0001 C CNN
	1    1650 2100
	1    0    0    -1  
$EndComp
$Comp
L power:+5VD #PWR033
U 1 1 5A4845D7
P 1650 3300
F 0 "#PWR033" H 1650 3150 50  0001 C CNN
F 1 "+5VD" H 1664 3480 50  0000 C CNN
F 2 "" H 1650 3300 50  0001 C CNN
F 3 "" H 1650 3300 50  0001 C CNN
	1    1650 3300
	1    0    0    -1  
$EndComp
$Comp
L power:+5VD #PWR031
U 1 1 5A490326
P 1650 900
F 0 "#PWR031" H 1650 750 50  0001 C CNN
F 1 "+5VD" H 1664 1080 50  0000 C CNN
F 2 "" H 1650 900 50  0001 C CNN
F 3 "" H 1650 900 50  0001 C CNN
	1    1650 900 
	1    0    0    -1  
$EndComp
Wire Wire Line
	1650 900  1650 950 
Wire Wire Line
	1650 950  1600 950 
$Comp
L power:PWR_FLAG #FLG01
U 1 1 5A49204E
P 1700 950
F 0 "#FLG01" H 1700 1025 50  0001 C CNN
F 1 "PWR_FLAG" V 1700 1083 50  0000 L CNN
F 2 "" H 1700 950 50  0001 C CNN
F 3 "" H 1700 950 50  0001 C CNN
	1    1700 950 
	0    1    1    0   
$EndComp
Wire Wire Line
	1700 950  1650 950 
Connection ~ 1650 950 
Text Label 7050 2300 2    50   ~ 0
USB2A+
Text Label 7050 2500 2    50   ~ 0
USB2B+
Text Label 7050 2600 2    50   ~ 0
USB2B-
Text Label 7050 2400 2    50   ~ 0
USB2A-
Text Label 7025 3475 2    50   ~ 0
USB3A+
Text Label 7025 3675 2    50   ~ 0
USB3B+
Text Label 7025 3775 2    50   ~ 0
USB3B-
Text Label 7025 3575 2    50   ~ 0
USB3A-
Text Label 7050 2100 2    50   ~ 0
USB2+
Text Label 7050 2200 2    50   ~ 0
USB2-
Text Label 7025 3275 2    50   ~ 0
USB3+
Text Label 7025 3375 2    50   ~ 0
USB3-
$Comp
L Custom:TS3USB3000 U?
U 1 1 5A4AF2BE
P 7500 1150
F 0 "U?" H 7500 1673 50  0000 C CNN
F 1 "TS3USB3000" H 7500 1580 50  0000 C CNN
F 2 "" H 6850 700 50  0001 C CNN
F 3 "" H 6850 700 50  0001 C CNN
	1    7500 1150
	1    0    0    -1  
$EndComp
Wire Wire Line
	7950 1450 7950 1400
Wire Wire Line
	7950 1400 7925 1400
Wire Wire Line
	7950 1400 7950 1200
Wire Wire Line
	7950 1200 7925 1200
Connection ~ 7950 1400
Wire Wire Line
	7975 850  7975 900 
Wire Wire Line
	7975 900  7925 900 
Wire Wire Line
	8300 1200 7950 1200
Connection ~ 7950 1200
Wire Wire Line
	8300 900  7975 900 
Connection ~ 7975 900 
Text GLabel 7900 2300 2    50   Input ~ 0
CTRL
$Comp
L power:GND #PWR?
U 1 1 5A4C4EFF
P 7925 2650
F 0 "#PWR?" H 7925 2400 50  0001 C CNN
F 1 "GND" V 7930 2518 50  0000 R CNN
F 2 "" H 7925 2650 50  0001 C CNN
F 3 "" H 7925 2650 50  0001 C CNN
	1    7925 2650
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR?
U 1 1 5A4C4F05
P 7950 2050
F 0 "#PWR?" H 7950 1900 50  0001 C CNN
F 1 "VCC" H 7967 2230 50  0000 C CNN
F 2 "" H 7950 2050 50  0001 C CNN
F 3 "" H 7950 2050 50  0001 C CNN
	1    7950 2050
	1    0    0    -1  
$EndComp
$Comp
L device:C C?
U 1 1 5A4C4F0B
P 8275 2250
F 0 "C?" V 8020 2250 50  0000 C CNN
F 1 "0.1uF" V 8113 2250 50  0000 C CNN
F 2 "Capacitors_SMD:C_0805_HandSoldering" H 8313 2100 50  0001 C CNN
F 3 "" H 8275 2250 50  0001 C CNN
	1    8275 2250
	-1   0    0    1   
$EndComp
$Comp
L Custom:TS3USB3000 U?
U 1 1 5A4C4F12
P 7475 2350
F 0 "U?" H 7475 2873 50  0000 C CNN
F 1 "TS3USB3000" H 7475 2780 50  0000 C CNN
F 2 "" H 6825 1900 50  0001 C CNN
F 3 "" H 6825 1900 50  0001 C CNN
	1    7475 2350
	1    0    0    -1  
$EndComp
Wire Wire Line
	7925 2650 7925 2600
Wire Wire Line
	7925 2600 7900 2600
Wire Wire Line
	7925 2600 7925 2400
Wire Wire Line
	7925 2400 7900 2400
Connection ~ 7925 2600
Wire Wire Line
	7950 2050 7950 2100
Wire Wire Line
	7950 2100 7900 2100
Wire Wire Line
	8275 2400 7925 2400
Connection ~ 7925 2400
Wire Wire Line
	8275 2100 7950 2100
Connection ~ 7950 2100
Text GLabel 7875 3475 2    50   Input ~ 0
CTRL
$Comp
L power:GND #PWR?
U 1 1 5A4C57EE
P 7900 3825
F 0 "#PWR?" H 7900 3575 50  0001 C CNN
F 1 "GND" V 7905 3693 50  0000 R CNN
F 2 "" H 7900 3825 50  0001 C CNN
F 3 "" H 7900 3825 50  0001 C CNN
	1    7900 3825
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR?
U 1 1 5A4C57F4
P 7925 3225
F 0 "#PWR?" H 7925 3075 50  0001 C CNN
F 1 "VCC" H 7942 3405 50  0000 C CNN
F 2 "" H 7925 3225 50  0001 C CNN
F 3 "" H 7925 3225 50  0001 C CNN
	1    7925 3225
	1    0    0    -1  
$EndComp
$Comp
L device:C C?
U 1 1 5A4C57FA
P 8250 3425
F 0 "C?" V 7995 3425 50  0000 C CNN
F 1 "0.1uF" V 8088 3425 50  0000 C CNN
F 2 "Capacitors_SMD:C_0805_HandSoldering" H 8288 3275 50  0001 C CNN
F 3 "" H 8250 3425 50  0001 C CNN
	1    8250 3425
	-1   0    0    1   
$EndComp
$Comp
L Custom:TS3USB3000 U?
U 1 1 5A4C5801
P 7450 3525
F 0 "U?" H 7450 4048 50  0000 C CNN
F 1 "TS3USB3000" H 7450 3955 50  0000 C CNN
F 2 "" H 6800 3075 50  0001 C CNN
F 3 "" H 6800 3075 50  0001 C CNN
	1    7450 3525
	1    0    0    -1  
$EndComp
Wire Wire Line
	7900 3825 7900 3775
Wire Wire Line
	7900 3775 7875 3775
Wire Wire Line
	7900 3775 7900 3575
Wire Wire Line
	7900 3575 7875 3575
Connection ~ 7900 3775
Wire Wire Line
	7925 3225 7925 3275
Wire Wire Line
	7925 3275 7875 3275
Wire Wire Line
	8250 3575 7900 3575
Connection ~ 7900 3575
Wire Wire Line
	8250 3275 7925 3275
Connection ~ 7925 3275
$EndSCHEMATC
